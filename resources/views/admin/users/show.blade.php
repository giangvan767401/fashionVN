            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <div class="px-8 pb-8">
                    ...
                    <div class="flex flex-col md:flex-row md:items-end -mt-12 mb-8 md:space-x-6">
                        <div class="w-24 h-24 bg-white p-1 rounded-2xl shadow-md border border-gray-100 overflow-hidden">
                            @if($user->avatar_url)
                                <img src="{{ $user->avatar_url }}" alt="{{ $user->full_name }}" class="w-full h-full object-cover rounded-xl">
                            @else
                                <div class="w-full h-full bg-indigo-50 flex items-center justify-center text-indigo-700 text-3xl font-bold rounded-xl">
                        ...
                        <div class="mt-4 md:mt-0 flex-1">
                            ...
                            <div class="flex items-center text-sm text-gray-500 mt-1">
                                <span class="px-2 py-0.5 {{ $user->role_id == 1 ? 'bg-purple-50 text-purple-600' : 'bg-blue-50 text-blue-600' }} rounded text-[10px] font-bold uppercase mr-3">
                                    {{ $user->role->name ?? 'User' }}
                                </span>
                                <span>ID: #USR-{{ str_pad($user->id, 4, '0', STR_PAD_LEFT) }}</span>
                            </div>
                        </div>
