...
<head>
    ...
    <title>{{ __('Thanh Toán') }} – Lumiere</title>
        ...
        <div style="display: flex; justify-content: center; align-items: flex-start;">
            <div style="padding: 48px 56px; max-width: 640px; width: 100%;">
                ...
                <div style="display: flex; align-items: center; gap: 10px; font-size: 13px; margin-bottom: 40px;">
                    ...
                    <span style="color: #d1d5db;">/</span>
                    <a href="{{ route('checkout.shipping') }}" style="color: #9ca3af; text-decoration: none;">{{ __('Vận Chuyển') }}</a>
                    <span style="color: #d1d5db;">/</span>
                    <span style="color: #111827; font-weight: 600;">{{ __('Thanh Toán') }}</span>
                </div>
                ...
                <div style="border: 1px solid #e5e7eb; border-radius: 6px; padding: 0 20px; margin-bottom: 40px;">
                    ...
                    <div class="info-row" style="border-bottom: none;">
                        <span class="info-label">{{ __('Phương Thức') }}</span>
                        <span class="info-value">
                            {{ session('checkout.shipping_method') === 'express' || session('checkout.shipping_fee') > 0 ? __('Giao Hàng Nhanh') : __('Chuyển Phát Tiêu Chuẩn') }}
                            · <span style="font-weight: 600;">{{ session('checkout.shipping_fee') > 0 ? number_format(session('checkout.shipping_fee'), 0, ',', '.') . 'đ' : __('Miễn Phí') }}</span>
                        </span>
                        <a href="{{ route('checkout.shipping') }}" class="info-action">{{ __('Thay Đổi') }}</a>
                    </div>
                </div>
                ...
                <form method="POST" action="{{ route('checkout.store_payment') }}">
                    ...
                    <h2 style="font-size: 18px; font-weight: 600; color: #111827; margin-bottom: 4px;">{{ __('Thanh Toán') }}</h2>
                    <p style="font-size: 13px; color: #6b7280; margin-bottom: 24px;">{{ __('Tất cả giao dịch đều được bảo mật và mã hóa.') }}</p>
                    ...
                    <div style="display: flex; flex-direction: column; gap: 12px;">
                        <label class="radio-box" id="label-cod">
                            ...
                            <div style="flex: 1; display: flex; flex-direction: column;">
                                <span style="font-size: 14px; font-weight: 600; color: #111827;">{{ __('Thanh Toán Khi Nhận Hàng (COD)') }}</span>
                                <span style="font-size: 12px; color: #6b7280; margin-top: 4px;">{{ __('Bạn sẽ thanh toán bằng tiền mặt khi đơn hàng được giao đến.') }}</span>
                            </div>
                        ...
                        <label class="radio-box" id="label-momo" style="border-color: #e5e7eb;">
                            ...
                            <div style="flex: 1; display: flex; flex-direction: column;">
                                <span style="font-size: 14px; font-weight: 600; color: #111827;">{{ __('Ví MoMo') }}</span>
                                <span style="font-size: 12px; color: #6b7280; margin-top: 4px;">{{ __('Thanh toán nhanh qua ví điện tử MoMo, quét QR hoặc nhập số điện thoại.') }}</span>
                            </div>
